<main class="mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-7 lg:px-8">
    ...
    <div class="grid gap-6 {{ auth()->user()->role->value === 'viewer' ? '' : 'lg:grid-cols-[360px_1fr]' }}">
        ...
        <section class="rounded-[10px] bg-white p-4 sm:p-5">
            ...
            <div class="space-y-3">@forelse($robots as $robot)<article class="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-[#fafafb] p-4"><div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><h3 class="font-extrabold">{{ $robot->name }}</h3><span class="rounded-lg px-2 py-1 text-xs font-bold {{ $robot->is_active ? 'bg-[#605bff]/10 text-[#605bff]' : 'bg-stone-100 text-stone-500' }}">{{ $robot->is_active ? 'Активен' : 'Отключён' }}</span></div><p class="mt-1 text-sm text-[#030229]/45">{{ $robot->description ?: 'Без описания' }} · {{ $robot->account?->name ?: 'Счёт не настроен' }} · {{ $robot->tracking_started_at ? 'Учёт с '.$robot->tracking_started_at->format('d.m.Y') : 'Без ограничения по дате' }}</p></div><div class="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap"><a href="{{ route('robots.show', $robot) }}" class="btn-primary text-center">Открыть</a>@if(auth()->user()->role->value !== 'viewer')<button wire:click="edit({{ $robot->id }})" class="btn-secondary">Изменить</button><button wire:click="toggle({{ $robot->id }})" class="btn-secondary">{{ $robot->is_active ? 'Отключить' : 'Включить' }}</button>@endif @if(auth()->user()->role->value === 'admin')<button wire:click="confirmDelete({{ $robot->id }})" class="btn-secondary text-red-700">Удалить</button>@endif</div></article>@empty<p class="rounded-xl bg-stone-50 p-5 text-sm text-stone-500">Роботов пока нет.</p>@endforelse</div>
        </section>
    </div>
    @if($deletingId)
