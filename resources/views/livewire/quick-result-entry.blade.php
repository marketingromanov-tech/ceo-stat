            ...
            <form wire:submit="save" x-init="$nextTick(() => $refs.{{ $robotId ? 'amount' : 'robot' }}?.focus())" class="relative z-10 w-full max-w-lg rounded-xl bg-white p-5 shadow-2xl sm:p-6">
                ...
                <div class="mt-5 space-y-4">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div><label class="form-label">Результат</label><div class="flex items-center gap-2"><button type="button" wire:click="toggleAmountSign" class="grid size-11 shrink-0 place-items-center rounded-lg border border-[#605bff]/20 bg-[#605bff]/10 text-xl font-extrabold text-[#605bff] transition hover:bg-[#605bff]/15 active:scale-95" aria-label="Изменить знак результата">−</button><input wire:model="amount" x-ref="amount" inputmode="decimal" class="form-input min-w-0 flex-1" placeholder="Например, -9,03"></div>@error('amount')<p class="mt-1 text-xs font-bold text-red-700">{{ $message }}</p>@enderror</div>
                    </div>
