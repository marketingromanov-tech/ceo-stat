<main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    ...
    <section class="mb-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article class="relative overflow-hidden rounded-[10px] bg-[#605bff] p-5 text-white"><span class="absolute -right-5 -top-5 size-24 rounded-full bg-white/10"></span><p class="text-xs font-bold uppercase tracking-wider text-white/60">Итого за всё время</p><p class="mt-3 text-3xl font-extrabold tracking-tight">{{ number_format($allTimeProfit, 2, ',', ' ') }}</p><p class="mt-2 text-xs text-white/60">Общий финансовый результат</p></article>
    ...
    <section class="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article class="relative overflow-hidden rounded-[10px] bg-[#030229] p-5 text-white"><span class="absolute -right-5 -top-5 size-24 rounded-full bg-white/10"></span><p class="text-xs font-bold uppercase tracking-wider text-white/60">Итого за месяц</p><p class="mt-3 text-3xl font-extrabold tracking-tight">{{ number_format($monthProfit, 2, ',', ' ') }}</p><p class="mt-2 text-xs text-white/60">Финансовый результат</p></article>
